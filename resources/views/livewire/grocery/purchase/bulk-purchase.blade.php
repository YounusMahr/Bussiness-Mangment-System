<div class="p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="hidden print-only">
            <div class="print-header">
                <h1>{{ __('messages.stock_purchases_management') }}</h1>
                <p>{{ __('messages.manage_your_stock_purchases') }}</p>
                ...
                <tbody>
                    @foreach($purchases as $index => $purchase)
                        <tr>
                            ...
                            <td>{{ number_format($purchase->total_stock, 2) }}</td>
                            <td>{{ number_format($purchase->given_stock, 2) }}</td>
                            <td>{{ number_format($purchase->remaining_stock, 2) }}</td>
                            <td>{{ $purchase->date->format('Y-m-d') }}</td>
                            <td>{{ $purchase->status === 'complete' ? __('messages.complete') : __('messages.incomplete') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <!-- Modal -->
    @if($showModal)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 no-print">
