    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex items-start justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">{{ __('add_new_installment') }}</h1>
                    <p class="text-slate-600 mt-1">{{ __('add_new_amount_to_existing_installment') }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-soft-xl overflow-hidden mb-6">
            ...
            <div class="p-6">
                <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ __('current_installment_information') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="text-sm font-medium text-slate-500">{{ __('current_car_price') }}</label>
                        <p class="text-base font-semibold text-blue-600">Rs {{ number_format($installment->car_price, 2) }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-slate-500">{{ __('current_interest') }}</label>
                        <p class="text-base font-semibold text-purple-600">Rs {{ number_format($installment->interest, 2) }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-slate-500">{{ __('current_total_price') }}</label>
        ...
        <div class="bg-white rounded-2xl shadow-soft-xl overflow-hidden">
            ...
            <form wire:submit.prevent="save" class="p-6 md:p-8">
                ...
                <div class="mb-8">
                    <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-4">{{ __('new_installment_details') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="new_car_price" class="block text-sm font-medium text-slate-700 mb-2">{{ __('new_car_price') }} *</label>
                            <div class="relative">
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('will_be_added_to_existing') }}</p>
                            @error('new_car_price') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label for="new_interest" class="block text-sm font-medium text-slate-700 mb-2">{{ __('new_interest') }} ({{ __('optional') }})</label>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-slate-400">Rs</span>
                        ...
                        <div>
                            <label for="new_total_price" class="block text-sm font-medium text-slate-700 mb-2">{{ __('new_total_price') }}</label>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-slate-400">Rs</span>
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('calculated_automatically') }} ({{ __('new_car_price') }} + {{ __('new_interest') }})</p>
                            @error('new_total_price') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label for="new_paid" class="block text-sm font-medium text-slate-700 mb-2">{{ __('new_paid') }} *</label>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-slate-400">Rs</span>
                        ...
                        <div>
                            <label for="new_remaining" class="block text-sm font-medium text-slate-700 mb-2">{{ __('new_remaining') }}</label>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-slate-400">Rs</span>
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('calculated_automatically') }} ({{ __('current_remaining') }} + {{ __('new_remaining_from_new_amount') }})</p>
                ...
                <div class="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6">
                    <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-4">{{ __('summary_after_addition') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                            <label class="text-sm font-medium text-slate-500">{{ __('total_car_price') }}</label>
                            <p class="text-lg font-bold text-blue-600">Rs {{ number_format($current_car_price + $new_car_price, 2) }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-slate-500">{{ __('total_interest') }}</label>
                        ...
                        <div>
                            <label class="text-sm font-medium text-slate-500">{{ __('total_paid') }}</label>
                ...
                <div class="pt-2 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-500 hover:from-blue-700 hover:to-indigo-600 text-white rounded-lg">{{ __('add_new_installment') }}</button>
                </div>
            </form>
        </div>
    </div>
