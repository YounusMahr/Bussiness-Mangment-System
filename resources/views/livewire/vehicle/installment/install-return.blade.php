    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex items-start justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">{{ __('messages.add_return_payment') }}</h1>
                    <p class="text-slate-600 mt-1">{{ __('messages.add_return_payment_for_installment') }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-soft-xl overflow-hidden mb-6">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="text-sm font-medium text-slate-500">{{ __('messages.phone_number') }}</label>
        ...
        <div class="bg-white rounded-2xl shadow-soft-xl overflow-hidden">
            ...
            <form wire:submit.prevent="save" class="p-6 md:p-8">
                ...
                <div class="mb-8">
                    <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-4">{{ __('messages.return_payment_details') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('messages.from_installment') }}</p>
                            @error('total_price') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label for="return_payment" class="block text-sm font-medium text-slate-700 mb-2">{{ __('messages.return_payment') }} *</label>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-slate-400">Rs</span>
                        ...
                        <div>
                            <label for="installment_number" class="block text-sm font-medium text-slate-700 mb-2">{{ __('messages.installment_number') }} ({{ __('messages.optional') }})</label>
                            <div class="relative">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('messages.calculated_automatically') }}</p>
                ...
                <div class="pt-2 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600 text-white rounded-lg">{{ __('messages.save_return_payment') }}</button>
                </div>
            </form>
        </div>
    </div>
